    ...
    t3 = () => {
      if (result?.success) {
        ...
        if (result && !result.success) {
          // Skip preflight failure — continue startup regardless
          logError(new Error(`Preflight check skipped: ${result.error}`));
          onSuccess();
        }
      }
    };
...
function _temp() {
  // no-op: preflight failure no longer exits the process
}
